...
# changing hidden_layer_size to 64 was needed to get compiler to work
...
#next line is new (stacking multiple LSTMs)
# num_LSTM_layers = 2
...
with tf.variable_scope("lstm"):
    ...
    cell = tf.contrib.rnn.MultiRNNCell([lstm_cell]*num_LSTM_layers, state_is_tuple=True)
    outputs, states = tf.nn.dynamic_rnn(cell, embed, sequence_length = _seqlens, dtype=tf.float32)
...
# Extract the final state and use in a linear layer
final_output = tf.matmul(states[num_LSTM_layers-1][1], weights["linear_layer"]) + biases["linear_layer"]
softmax = tf.nn.softmax_cross_entropy_with_logits(labels=_labels, logits=final_output)
